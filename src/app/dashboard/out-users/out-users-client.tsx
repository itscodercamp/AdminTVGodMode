...
type OutUsersClientProps = {
  initialUsers: Omit<User, 'password'>[];
  initialDealers: Dealer[];
};

export function OutUsersClient({ initialUsers, initialDealers }: OutUsersClientProps) {
  const [deletedUsers, setDeletedUsers] = React.useState<Omit<User, 'password'>[]>(initialUsers);
  const [deletedDealers, setDeletedDealers] = React.useState<Dealer[]>(initialDealers);
